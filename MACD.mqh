...
private:
   ...
   double            m_val[MACD_MODES][TFS][MACD_VALUES];
   int               m_last_error;
   ...
   bool              Update(const ENUM_TIMEFRAMES _tf=PERIOD_CURRENT)
     {
      ...
      if(CopyBuffer(m_params.handles[index],MAIN,m_params.shift,MACD_VALUES,MaArray)!=MACD_VALUES)
         return(false);
      m_val[MAIN][index][CUR]=MaArray[2];
      m_val[MAIN][index][PREV]=MaArray[1];
      m_val[MAIN][index][FAR]=MaArray[0];

      if(CopyBuffer(m_params.handles[index],SIGNAL,m_params.shift,MACD_VALUES,MaArray)!=MACD_VALUES)
         return(false);
      m_val[SIGNAL][index][CUR]=MaArray[2];
      m_val[SIGNAL][index][PREV]=MaArray[1];
      m_val[SIGNAL][index][FAR]=MaArray[0];
...
public:
   ...
   bool              SetParams(const string symbol,
                               ...
                               )
     {
      ...
      for(int i=0;i<TFS;i++)
        {
         m_params.handles[i]=iMACD(m_params.symbol,
                                   tf[i],
                                   m_params.fast_period,
                                   m_params.slow_period,
                                   m_params.signal_period,
         ...
         if(_cmd==TRADE_BUY)
           {
            if((_open_method&OPEN_METHOD_1)==OPEN_METHOD_1)
               result[i]=m_val[MAIN][index][CUR]<0.0 && 
                         m_val[MAIN][index][PREV]<m_val[SIGNAL][index][PREV] && 
                         m_val[MAIN][index][CUR]>m_val[SIGNAL][index][CUR]+level;
            //---
            if((_open_method&OPEN_METHOD_2)==OPEN_METHOD_2)
               result[i]=m_val[MAIN][index][PREV]<0 && 
                         m_val[MAIN][index][CUR]>=0;
            //---
